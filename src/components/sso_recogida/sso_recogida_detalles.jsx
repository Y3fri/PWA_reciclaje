...
import ModalFecha from './Modales/Modalfecha';
...
const SsoRecogidaDetalles = () => {
    ...
    const openModalFecha = (ssorecogidaId) => {
    ...
    const updatessorecogidaList = async () => {
        await listsso_recogida(setsso_recogidas);
    };
                                    ...
                                    <tbody>
                                        {currentssorecogidas.map((sso_recogida, index) => (
                                            <tr key={sso_recogida.sreg_id || index}>
                                                ...
                                                <td>{sso_recogida.sreg_fecha === '2024-12-23' ? 'NULL' : sso_recogida.sreg_fecha}</td>
                                                <td>{sso_recogida.sreg_hora1 === '23:12:12' ? 'NULL' : sso_recogida.sreg_hora1}</td>
                                                <td>{sso_recogida.sreg_hora2 === '23:12:12' ? 'NULL' : sso_recogida.sreg_hora2}</td>

                                                <td>
                                                    <button className="button-edit" onClick={() => openModal(sso_recogida.sreg_id)}>Detalles</button>
                                                    {!(sso_recogida.sreg_fecha === '2024-12-23' && sso_recogida.sreg_hora1 === '23:12:12' && sso_recogida.sreg_hora2 === '23:12:12') && (
                                                        <button className="button-edit" onClick={() => openModalFecha(sso_recogida.sreg_id)}>Editar Fecha</button>
                                                    )}
                                                </td>
                                            </tr>
                                        ))}
